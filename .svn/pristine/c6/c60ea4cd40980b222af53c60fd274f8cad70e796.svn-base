...
<!--    action="http://192.168.0.133:9001/jjkj/sfj/api/support/uploadFile"-->
    <el-upload
      ...
      action="http://192.168.0.133:9001/jjkj/sfj/api/support/uploadFile"
      accept=".doc,.docx"
      :on-preview="handlePreview" :on-remove="handleRemove" :on-success="handleSuccess" :on-error="handleError" :on-progress="handleProgress" :on-change="fileChange"
      :before-remove="beforeRemove" multiple
      :on-exceed="handleExceed" :file-list="fileList" :data="dataFrom"
      name="file"
      >
...
//这里存放数据
      ...
      methods: {
        ...
        handleSuccess(response, file, fileList){
          ...
          this.$emit('submitInformchild',this.imgId,file.raw.name,this.fileSize);
        },
        handleError(){
